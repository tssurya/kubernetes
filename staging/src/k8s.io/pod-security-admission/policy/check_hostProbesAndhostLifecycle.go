...
const (
	...
	allowedLocalHostName = "localhost"
)

var allowedHostValues = sets.New[string](allowedLocalHostIPv4, allowedLocalHostIPv6, allowedLocalHostName)
...
// the only allowed values are `127.0.0.1`, `::1`, and `localhost`
...
func getForbiddenHostProbes(probe *corev1.Probe) []string {
	...
	if probe.HTTPGet != nil && probe.HTTPGet.Host != "" && !allowedHostValues.Has(probe.HTTPGet.Host) {
		badHosts = append(badHosts, probe.HTTPGet.Host)
	}
	if probe.TCPSocket != nil && probe.TCPSocket.Host != "" && !allowedHostValues.Has(probe.TCPSocket.Host) {
...
func getForbiddenHostLifecycle(handler *corev1.LifecycleHandler) []string {
	...
	if handler.HTTPGet != nil && handler.HTTPGet.Host != "" && !allowedHostValues.Has(handler.HTTPGet.Host) {
		badHosts = append(badHosts, handler.HTTPGet.Host)
	}
	if handler.TCPSocket != nil && handler.TCPSocket.Host != "" && !allowedHostValues.Has(handler.TCPSocket.Host) {
